...
def preproc_fast(image, sigma):    
    mean = np.average(image)
    threshold = np.std(image)*sigma
    above = np.where(image > mean+threshold,image,0)
    # above = np.where(image > mean+threshold*2,1,0.5)       
    return above

def rotate_source(split, partition, labels_aug, label, source, rotate_factor, data_path, label_map, extension=".fits",sub_class_map={},num_classes=4, img_rows=300, img_cols=300):
...
def make_splits(rotate_factor, data_path, data_manifest, seed, train_size=250,val_size=250, img_rows=300,img_cols=300):
    ...
    for key in keys:
        ...
        if train[labels[key]] < train_size:
            partition, labels_aug, skip = rotate_source("train", partition, labels_aug, labels[key], key, rotate_factor, data_path, label_map[labels[key]+1], sub_class_map=sub_class_map, num_classes=num_classes,img_rows=img_rows,img_cols=img_cols)
            if skip == False:
                train[labels[key]]+=1
            else:
                continue
        ...
        #         continue
            ...
            partition, labels_aug, skip = rotate_source("validation", partition, labels_aug, labels[key], key, rotate_factor, data_path, label_map[labels[key]+1], sub_class_map=sub_class_map, num_classes=num_classes,img_rows=img_rows,img_cols=img_cols)
            if skip == False:
